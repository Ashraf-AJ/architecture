...
@pytest.fixture
def bus(in_memory_session_factory):
...
def get_email_from_mailhog(sku):
    host, port = map(
        config.get_email_host_and_port().get, ["host", "http_port"]
    )
...
def test_out_of_stock_email(bus):
    ...
    assert email["Raw"]["From"] == "allocation@example.com"
    assert email["Raw"]["To"] == ["test@example.com"]
    assert f"out of stock {sku}" in email["Raw"]["Data"]
